...
# ImageMagick path setting only for Windows (for TextClip usage)
...
with gr.Blocks(theme="soft") as demo:
    ...
    with gr.Tabs():
        ...
        with gr.TabItem("🖼️ Image Editor"):
            ...
            brightness = gr.Slider(-100, 100, value=0, label="Brightness", interactive=True)
            contrast = gr.Slider(-100, 100, value=0, label="Contrast", interactive=True)
            hue = gr.Slider(-100, 100, value=0, label="Hue", interactive=True)
            saturation = gr.Slider(-100, 100, value=0, label="Saturation", interactive=True)
            flip = gr.Checkbox(label="Flip Image")
            grayscale = gr.Checkbox(label="Convert to Black & White")
            rotate = gr.Slider(0, 180, value=0, label="Rotate", step=1)
    ...
    # Live update with slider/checkbox change
    for inp in inputs:
        inp.change(process_image, inputs=inputs, outputs=output_image, show_progress=False)

    # Reset all values
    clear_button.click(fn=reset_parameters, outputs=[brightness, contrast, hue, saturation, flip, grayscale, rotate, blur, sharpen, crop])
